...
class CourtListener(SearchTarget):
    ...
    def search(search_statement: str):
        ...
        #
        headers = {
            "User-Agent": "OLAW/0.0.4 (https://github.com/harvard-lil/olaw)"
        }
        if os.environ.get("COURT_LISTENER_API_TOKEN"):
            headers["Authorization"] = f"Token {os.environ['COURT_LISTENER_API_TOKEN']}"
        ...
        try:
            ...
            if res.status_code != 200:
                print(f"CourtListener search error ({res.status_code}): {res.text[:200]}")
                return []
            raw_results = res.json()
        except Exception:
            return []

        if not isinstance(raw_results, dict) or "results" not in raw_results:
            return []
        ...
        for i in range(0, max_results):
            ...
            # Case-specific data
            opinion["id"] = opinion_metadata.get("id", "")
            opinion["case_name"] = opinion_metadata.get("caseName", "")
            opinion["court"] = opinion_metadata.get("court", "")
            opinion["absolute_url"] = base_url + opinion_metadata.get("absolute_url", "")
            opinion["status"] = opinion_metadata.get("status", "")
            opinion["date_filed"] = opinion_metadata.get("dateFiled", "")
