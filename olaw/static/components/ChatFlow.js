...
export class ChatFlow extends HTMLElement {
   ...
   */
  ask = async () => {
    // Remove placeholder or welcome screen if still present
    this.querySelector(".placeholder")?.remove();
    this.querySelector(".welcome-screen")?.remove();
  ...
  renderInnerHTML = () => {
    this.innerHTML = /*html*/ `
      <div class="welcome-screen">
        <div class="hero-header">
          <h1 class="hero-title">
            <span class="animated-gradient">aiKart Legal AI Workbench</span>
          </h1>
          <p class="hero-subtitle">
            Autonomous Retrieval Augmented Generation (RAG) powered by Gemini & CourtListener Case Law APIs.
          </p>
        </div>

        <div class="quick-actions">
          <button class="glass-pill action-chip" data-prompt="What are the legal precedents for Miranda v. Arizona (1966)?">
            📜 <span>Miranda v. Arizona</span>
          </button>
          <button class="glass-pill action-chip" data-prompt="What are the legal precedents for self-defense in castle doctrine states?">
            🛡️ <span>Castle Doctrine</span>
          </button>
          <button class="glass-pill action-chip" data-prompt="What is the Supreme Court holding in District of Columbia v. Heller?">
            🏛️ <span>D.C. v. Heller</span>
          </button>
        </div>

        <div class="recommendations-grid">
          <div class="glass-card prompt-card" data-prompt="Search CourtListener case law for Fourth Amendment digital search and seizure.">
            <div class="card-icon">🔍</div>
            <div class="card-content">
              <h3>Search Precedent Case Law</h3>
              <p>Analyze Fourth Amendment rights regarding digital search and seizure APIs.</p>
            </div>
          </div>

          <div class="glass-card prompt-card" data-prompt="Explain the key legal elements required for proving breach of fiduciary duty.">
            <div class="card-icon">⚖️</div>
            <div class="card-content">
              <h3>Fiduciary Duty Elements</h3>
              <p>Extract core legal requirements and court standards for fiduciary duty cases.</p>
            </div>
          </div>
        </div>
      </div>
    ...
    // Add click listeners to quick actions and prompt cards
    this.querySelectorAll("[data-prompt]").forEach((el) => {
      el.addEventListener("click", () => {
        const promptText = el.getAttribute("data-prompt");
        state.message = promptText;
